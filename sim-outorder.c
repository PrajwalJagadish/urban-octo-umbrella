...
// static int spu_queue_size;

// static int num_spu_queues;
...
//static void quicksort(struct RUU_station*, int* ,int ,int );
// static struct RUU_station** SPU_dump_ready_queues(void);
...
//   miss_queue = calloc(NUM_SPU_QUEUES, sizeof(RA));
//   if (!miss_queue) {
...
// static void
// init_depend_tree(unsigned int dest_reg) {
//   if (SPU_queues_in_use == SPU_QUEUE_SIZE) {
//     //stall (how?)
//     return -1;
//   }

//   for (int avail_queue = 0; avail_queue < SPU_QUEUE_SIZE, avail_queue++) {
//     if (miss_queue[avail_queue] == 0) 
//       break;
//   }
...
static 
void quicksort(struct RUU_station *ruus[], int insert_times[] ,int first,int last){
   int i, j, pivot, temp;
   struct RUU_station* temp_ruu;
   if(first<last){
      pivot=first;
      i=first;
      j=last;
      ...
      while(i<j){
         while(insert_times[i]<=insert_times[pivot]&&i<last)
            i++;
         while(insert_times[j]>insert_times[pivot])
            j--;
         if(i<j){
            temp=insert_times[i];
            insert_times[i]=insert_times[j];
            insert_times[j]=temp;
            temp_ruu = ruus[i];
            ruus[i] = ruus[j];
            ruus[j] = temp_ruu;
      ...
      temp=insert_times[pivot];
      insert_times[pivot]=insert_times[j];
      insert_times[j]=temp;
      ...
      quicksort(ruus, insert_times,first,j-1);
      quicksort(ruus, insert_times,j+1,last);
      return;
   }
}
  ...
  for(int i = 0; i<NUM_SPU_QUEUES; i++){
    if(SPU[i].initalized && SPU[i].finish == sim_cycle){
      if(ready_queues == NULL){
        ready_queues = malloc(sizeof(int));
        if(ready_queues == NULL) fatal("Out of virtual memory");
        ready_queues[0] = i;
        number_of_ruus += SPU[i].instructs_in_queue;
        number_of_queues_ready++;
      }else{
        ready_queues = realloc(ready_queues, (number_of_queues_ready + 1)*sizeof(int));
        if(ready_queues == NULL) fatal("Out of virtual memory");
        ready_queues[number_of_queues_ready] = i;
        number_of_ruus += SPU[i].instructs_in_queue;
        number_of_queues_ready++;
      }
    }
	  ...
	  if (rs->in_LSQ
	      ...
	      if (MD_OP_FUCLASS(rs->op) != NA)
		{
          ...
          if ((load_lat > cache_dl1_lat) && ((MD_OP_FLAGS(rs->op) & (F_MEM|F_LOAD))
		    ...
		    {
		      /*  functional unit resources, put operation
			 back onto the ready list, we'll try to issue itinsufficient
			 again next cycle */
		      readyq_enqueue(rs);
		    }
		}
